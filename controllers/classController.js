...
exports.createClassInAcademicYear = async (req, res) => {
    try {
        const academicYear = await AcademicYear.findByPk(req.params.id);
        if (!academicYear) return res.status(404).json({ message: 'Tahun ajaran tidak ditemukan' });

        const existingClass = await Class.findOne({
            where: { name: req.body.name, academic_year_id: req.params.id }
        });
...
exports.updateClass = async (req, res) => {
    try {
        ...
        if (!existingClass) return res.status(404).json({ message: 'Kelas tidak ditemukan' });
